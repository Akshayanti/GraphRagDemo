...
class KnowledgeBaseCreator:
    ...
    def create_knowledge_base(self):
        ...
        for graph_document in tqdm.tqdm(graph_documents, desc="Creating Knowledge Base"):
            node_id = graph_document.metadata.get("id", "unknown")
            logging.debug(f"Adding node: {node_id} with metadata: {graph_document.metadata}")
            self.graph_store.add_node(node_id, **graph_document.metadata)

            description = graph_document.metadata.get("Description", "unknown")
            logging.debug(f"Adding edge from {node_id} to {description} with relation 'has_description'")
            self.graph_store.add_edge(node_id, description, relation="has_description")

            impact = graph_document.metadata.get("Impact", "unknown")
            logging.debug(f"Adding edge from {node_id} to {impact} with relation 'has_impact'")
            ...
            logging.debug(f"Adding edge from {node_id} to {published_date} with relation 'published_on'")
            ...
            logging.debug(f"Adding edge from {node_id} to {assigner} with relation 'assigner'")
            self.graph_store.add_edge(node_id, assigner, relation="assigner")

            for problemtype in graph_document.metadata.get("ProblemType", []):
                logging.debug(f"Adding edge from {node_id} to {problemtype} with relation 'problem_type'")
    ...
    def get_graph_chain(self):
        return GraphQAChain.from_llm(llm=self.llm,
                                     graph=NetworkxEntityGraph(self.graph_store),
                                     verbose=True)
...
if __name__ == "__main__":
    kb_creator = KnowledgeBaseCreator(output_directory="./filtered_jsons", model="llama3.2:1b")
